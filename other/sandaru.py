...
def empty_spaces(grid):
    count = 0
    no = random.randint(0,(cols*rows)//2) #random number for the number of spaces in the grid 
    while count<no:
        index1 = random.randint(0,rows-1) #to select a the list
        index2 = random.randint(0,cols-1) #to select an element from the specific list 
        grid[index1][index2] = "  "
        count+=1
    return grid
...
if args:
    ...
    if len(dimensions) != 2:
        ...
        if not (3 <= rows <= 9 and 3 <= cols <= 9):
            raise ValueError("Dimensions out of range. Please choose between 3x3 and 9x9.")
else:
    rows, cols = 5, 5
